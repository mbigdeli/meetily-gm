...
async fn run_import<R: Runtime>(
    ...
    let use_parakeet = provider.as_deref() == Some("parakeet");
    let use_shenava = provider.as_deref() == Some("shenava");
    ...
    let whisper_engine = if !use_parakeet && !use_shenava && total_segments > 0 {
    ...
    let shenava_engine = if use_shenava && total_segments > 0 {
        Some(
            crate::audio::transcription::shenava_provider::engine_for_model(model.as_deref())
                .await
    ...
    for (i, segment) in processable_segments.iter().enumerate() {
        ...
        let (text, conf) = if use_parakeet {
            ...
            let text = shenava_engine
                .as_ref()
                .ok_or_else(|| anyhow!("Shenava engine is unavailable"))?
                .transcribe(segment.samples.clone())
                .await
                .map_err(|error| anyhow!("Shenava transcription failed on segment {i}: {error}"))?;
            (text, 0.9f32)
        } else {
